...
module.exports = {
  ...
  async execute(message, reply) {
    ...
    embed.addFields({
      name: "Similarity",
      value: `Reference image is ${response[0].similarity}% similar to the original image `,
    });
    ...
    if (
      typeof response[0].authorName === "string" ||
      response[0].authorName instanceof String
    ) {
    ...
    if (response[0].similarity > 70) {
      ...
      if (twitterURL.test(response[0].source)) {
        embed.addFields({ name: "Link", value: response[0].source });
      } else if (pixivURL.test(response[0].url)) {
        embed.addFields({ name: "Link", value: response[0].url });
      } else {
        embed.addFields({ name: "Link", value: response[0].url });
      }
    } else {
      embed.setTitle(
        `Deepest apologies, can't findeth thy sauce, this is the nearest one D:`
      );
